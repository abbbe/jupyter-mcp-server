...
All 14 built-in tools are instrumented with `BEFORE_TOOL_CALL` / `AFTER_TOOL_CALL` hooks. Cell and code execution tools additionally fire `BEFORE_EXECUTE` / `AFTER_EXECUTE` hooks around kernel interactions.
...
Resolution order: CLI argument → environment variable → not enabled.
